...
Là thuật ngữ chung để chỉ các cơ sở dữ liệu không sử dụng mô hình quan hệ truyền thống. NoSQL thực chất bao gồm nhiều loại cơ sở dữ liệu khác nhau, như **Document Store** (MongoDB, CouchDB), **Key-Value Store** ([Redis](/2026/04/06/elasticache#redis-vs-memcached), [DynamoDB](/2026/03/18/dynamodb)), **Column Family Store** (Cassandra, HBase), và **Graph Database** (Neo4j, Amazon Neptune).
