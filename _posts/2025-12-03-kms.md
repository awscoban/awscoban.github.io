...
categories: [KMS, Security]
tags: [KMS, Security]
img: /assets/9_kms/kms.png
summary: "Để hiểu cách S3 (và các dịch vụ khác) bảo mật dữ liệu, hãy tìm hiểu về dịch vụ mã hoá và giải mã trên AWS: Key Management Service (KMS). Trước hết hãy cùng ôn lại các khái niệm trong mật mã hoá."
---
...
AWS Managed Key, như tên gọi, được *tạo sẵn bởi AWS* khi sử dụng các dịch vụ cần mã hoá. Ví dụ, khi sử dụng tính năng Server-Side Encryption trong S3 và chọn để S3 quản lý khoá ([SSE-S3](/2025/12/06/s3-security#sse-s3)), S3 sẽ tạo một AWS Managed Key trong KMS có tên gọi (*alias*) là `aws/s3` với key policy phù hợp để sử dụng. Người dùng không cần thực hiện thao tác nào. AWS Managed Key được tự động xoay tua (**key rotation**), tức thay đổi key material (vẫn giữ nguyên ARN của KMS Key) để đáp ứng các yêu cầu bảo mật.
